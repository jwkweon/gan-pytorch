"""code from https://github.com/RenYurui/Global-Flow-Local-Attention/
    Usage : python script/fid.py --distorated_path 'gen_path' --fid_real_path 'gt_path' --name 'proj'
    ...
    def compute_statistics_of_path(self, path, verbose):
        ...
        if os.path.exists(npz_file):
            ...
            for fn in tqdm(files):
                im = imread(str(fn))
                im = np.array(Image.fromarray(im).resize((256, 256))).astype(np.float32)
                imgs.append(im)
...
if __name__ == "__main__":
    ...
    df = pd.DataFrame(dic)
    save_path = './eval_results/'+args.name+'.csv'
